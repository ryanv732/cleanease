...
class digistoreModeldigistoreCart extends obModel
{
	public $orders 		= array();
	...
	/**
	 * Get plugins type digistore payment list
	 *
	 * @return array
	 */
	function getPluginList()
	{
		...
		$plugins = JPluginHelper::getPlugin( 'digistorepayment' );
	...
	function addToCart($customer, &$productname){
		$db = JFactory::getDBO();
		$sid = $customer->_sid; //digistore session id
		...
		if(trim($renewlicid) != ""){
			$sql = "select `offerplans` from #__digistore_products where id=".intval($pid);
			$db->setQuery($sql);
			$offerplans = intval($db->loadResult());

			$sql = "select `expires` from #__digistore_licenses where id=".intval($renewlicid);
			$db->setQuery($sql);
		...
		if($plan_id == "-1"){
			$sql = "select `plan_id` from #__digistore_products_plans where product_id=".intval($pid)." and `default`=1";
			$db->setQuery($sql);
			$plan_id = intval($db->loadResult());
		}

		$sql = "select name, access from #__digistore_products where id=".intval($pid);
		$db->setQuery( $sql );
		$res = $db->loadObject();
		...
		$sql = "select cid, item_id, quantity from #__digistore_cart where sid='".intval($sid)."' AND item_id='".intval($pid)."' AND plan_id = '".intval($plan_id)."'";
		$db->setQuery($sql);
		...
		if(!$item_id){//no such item in cart- inserting new row
			...
			$sql = "insert into #__digistore_cart (quantity, item_id, sid, userid, plan_id, renew, renewlicid)"
			...
			foreach($data as $dat){ //for all items of the same product in the cart
				...
				$sql = "select fieldid, optionid from #__digistore_cartfields where cid='".intval($cid). "' and sid='".intval($sid)."' and productid='".intval($pid). "'";
				$db->setQuery($sql);
			...
			if($update_done != 1){//no matching record found
				$sql = "insert into #__digistore_cart (quantity,item_id,sid,userid) "
		...
		foreach($fields as $field){//now we've to update dependency entries
			...
			$sql = "delete from #__digistore_cartfields where sid='".intval($sid)."' and productid='".intval($pid)."' and fieldid='".intval($field->id)."' and cid='" . $cid . "'";
			$db->setQuery($sql);
			$db->query(); //remove old one regardless of they existence
			$sql = "insert into #__digistore_cartfields(cid, sid, productid, fieldid, optionid) values ('".intval($cid)."','".intval($sid)."','".intval($pid)."','". $field->id."', '".$value."')";
			$db->setQuery( $sql );
		...
		$sql = "select usestock as res from #__digistore_products where id='".intval($pid)."'";
		$db->setQuery($sql);
		$usestock = $db->loadResult();

		$sql = "select (stock-used) as res from #__digistore_products where id='".intval($pid)."'";
		$db->setQuery($sql);
		$stock = $db->loadResult();

		$sql = "select quantity from #__digistore_cart where item_id='".intval($pid)."' and sid='".intval($sid)."' and userid='".@$my->id."' and cid='".intval($cid). "'";
		$db->setQuery( $sql );
		$quant = $db->loadResult();

		if($stock < $quant && $usestock > 0 && $stock > 0){
			$sql = "update #__digistore_cart set quantity = ".$stock . ""
			. " where sid='".$sid."' AND item_id='".$item_id."' and cid='".intval($cid)."'";
			$db->setQuery($sql);
			$db->query(); //updating quantity
		}
		elseif($stock <= 0 && $usestock > 0){
			$sql = "delete from #__digistore_cart where cid='".intval($cid)."' and sid='".intval($sid)."' and item_id='".intval($pid)."'";
			$db->setQuery($sql);
			$db->query();
		}
	...
	function getCartItems($customer, $configs)
	{
		...
		$sql = "SELECT 
					...
					FROM
						`#__digistore_settings` AS `s`,
						`#__digistore_products` AS `p`
							INNER JOIN
						`#__digistore_cart` AS `c` ON (`c`.`item_id` = `p`.`id`)
							LEFT JOIN
						`#__digistore_plans` AS `pl` ON (`c`.`plan_id` = `pl`.`id`)
							LEFT JOIN
						`#__digistore_products_plans` AS `pp` ON (`pp`.`product_id` = `p`.`id`
							AND `pp`.`plan_id` = `pl`.`id`)
							LEFT JOIN
						`#__digistore_products_renewals` AS `ppr` ON (`ppr`.`product_id` = `p`.`id`
		...
		foreach ($items as $i => $item)
		{
			...
			if (!empty($new_plan_id))
			{
				...
				{
					$sql = "update #__digistore_cart set plan_id = ".$new_plan_id." where sid = ".intval($sid)." and cid=".$item->cid;
					$db->setQuery($sql);
					$db->query();
			...
			if (($item->plan_id < 1))
			{
				...
				if ($item->renew == 0 )
				{
					$sql = "SELECT pl.id as value, pl.name as text, pp.price, pp.default
							FROM #__digistore_products_plans pp
								 LEFT JOIN #__digistore_plans pl ON ( pp.plan_id = pl.id )
							WHERE pp.product_id = ".$item->id;
				}
				else
				{
					$sql = "SELECT pl.id as value, pl.name as text, pp.price, pp.default
							FROM #__digistore_products_renewals pp
								 LEFT JOIN #__digistore_plans pl ON ( pp.plan_id = pl.id )
							WHERE pp.product_id = " . $item->id;
				}
		...
		for ( $i = 0; $i < count( $items ); $i++ )
		{
			...
			$item->price_formated = digistoreHelper::format_price2( $item->price, $item->currency, false, $configs ); //sprintf( $price_format, $item->product_price );
			$item->subtotal_formated = digistoreHelper::format_price2( $item->subtotal, $item->currency, false, $configs ); //sprintf( $price_format, $item->subtotal );
		}
		...
		foreach ( $items as $i => $product ) {
			...
			$sql = "select f.size, f.name, f.options, f.id, fd.mandatory, fd.publishing
					from #__digistore_customfields f
						 left join #__digistore_prodfields fd on (f.id=fd.fieldid)" . (count( $where1 ) > 0 ? "
			...
			if ( count( $fields ) > 0 ) {
				...
				foreach ( $item->productfields as $j => $v ) {
					...
					$options = explode( "\n", $v->options );
					$sql = "select optionid from #__digistore_cartfields where fieldid='" . $v->id . "' and cid='" . $product->cid . "'";
					$db->setQuery( $sql );
			...
			$sql = "select f.featuredid as id, p.name as name, f.planid, pl.name as planname, pl.duration_count, pl.duration_type from #__digistore_featuredproducts f, #__digistore_products p, #__digistore_plans pl where f.planid = pl.id and f.featuredid=p.id and f.productid=".intval($product->id);
		...
		if(count($items) > 0){
			...
			if($configs->tax_price and false){
		...
		if(isset($items) && count($items) > 0){
			foreach($items as $key=>$value){
				if(isset($value) && isset($value->id)){
					$sql = "select `path`, `title` from #__digistore_products_images where product_id=".intval($value->id)." and `default`=1";
					$db->setQuery($sql);
					$db->query();
					$result = $db->loadAssocList();
					if(!isset($result) || count($result) == 0){
						$sql = "select `path`, `title` from #__digistore_products_images where product_id=".intval($value->id)." and `default`= 0 limit 1";
						$db->setQuery($sql);
						$db->query();
						$result = $db->loadAssocList();
					}
	...
	function calc_price(&$items, $cust_info, $configs)
	{
		...
		$tax_handler = $this->getInstance( "digistoreTax", "digistoreModel" );
		if (is_object($cust_info))	$sid = $cust_info->_sid;
		if (is_array($cust_info))	$sid = $cust_info['sid'];
		$customer = $cust_info;
		//		if (!$sid )	$sid = get_sid();

		if ( !isset( $configs->totaldigits ) ) {
		...
		$tax['shipping'] = digistoreHelper::format_price( $tax['shipping'], $tax['currency'], false, $configs ); //sprintf($price_format, $tax['shipping']);
		$tax['taxed'] = digistoreHelper::format_price( $tax['taxed'], $tax['currency'], false, $configs ); //sprintf($price_format, $tax['taxed']);//." ".$tax['currency'];
	...
	function havePreviousOrderOfProduct($promo)
	{
		...
		$sql = "SELECT COUNT(*)
				FROM `#__digistore_promocodes_orders` AS o
					 INNER JOIN `#__digistore_licenses` AS l ON l.`productid`=o.`productid`
				WHERE l.`published`=1
	...
	function get_promo( $customer, $checkvalid = 1 )
	{
		...
		$db = JFactory::getDBO();
		$sql = "select cart_details from #__digistore_session where sid='" . intval($sid) . "'";
		$db->setQuery( $sql );
		...
		if ( strlen( $promocode ) > 0 ) {//valid promocode was provided
			...
			$sql = "SELECT p.`productid`
					FROM `#__digistore_promocodes_products` AS p
			...
			$promo = $this->getTable( "digistorePromo" );
		}
		$promo->error = "";
		...
		if ( $promo->id > 0 && $checkvalid == 1 ) {
			if ( $uid > 0 ) {
				$sql = "select count(*) from #__digistore_licenses where userid='" . $uid . "' and published='1'";
				$db->setQuery( $sql );
	...
	function updateCart( $customer, $configs )
	{
		...
		$sql = "update #__digistore_session set processor='" . $processor . "' where sid='" . intval($sid) . "'";
		...
		foreach ( $items as $i => $item ) {
			...
			if ( !empty( $new_plan_id ) ) {
				...
				if ( $plan_id > 0 ) {
					$sql = "update #__digistore_cart set plan_id =".intval($plan_id)." where sid=".intval($sid)." and cid=".intval($item->cid);
					$db->setQuery( $sql );
					$db->query();
		...
		$sql = "select c.* from #__digistore_cart c where c.sid='".intval($sid)."'";
		$db->setQuery( $sql );
		$cartitems = $db->loadObjectList();

		//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
		foreach ( $cartitems as $i => $item ) {
			$sql = "select fieldid, optionid from #__digistore_cartfields where sid='" . intval($sid) . "' and cid='" . $item->cid . "'";
			$db->setQuery( $sql );
			$fields = $db->loadObjectList();
		...
		for ( $i = 0; $i < (sizeof( $cartitems ) - 1); ++$i ) {
			...
			for ( $i1 = $i; $i1 < sizeof( $cartitems ); ++$i1 ) {
				...
				/*
				  ...
				  $sql = "delete from #__digistore_cart where cid='".$item1->cid."' and sid='".$sid."'";
				  $db->setQuery($sql);
				  $db->query();
				  //delete field set for the removed entry
				  $sql = "delete from #__digistore_cartfields where cid='".$item1->cid."' and sid='".$sid."'";
				  $db->setQuery($sql);
				  $db->query();
				  //update first entry
				  $sql = "update #__digistore_cart set quantity=quantity+".$item1->quantity." where cid='".$item->cid."' and sid='".$sid."'";
				  $db->setQuery($sql);
				  $db->query();
				  }
				 */
		...
		if ( !empty( $replicate ) )
			foreach ( $replicate as $cid => $value ) {
				...
				if ( $value > 0 && $cid > 0 ) {
					$sql = "insert into #__digistore_cart (item_id, userid, quantity, sid) values ('" . $value . "', '" . $uid . "', '1', '" . intval($sid) . "')";
					$db->setQuery( $sql );
					$db->query();
					$incid = $db->insertid();
					//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');

					$sql = "select fieldid from #__digistore_prodfields where productid='" . $value . "' and publishing='1' ";
					$db->setQuery( $sql );
					$fields = $db->loadObjectList();
					foreach ( $fields as $field ) {
						$sql = "insert into #__digistore_cartfields(cid, sid, productid, fieldid, optionid) values ('" . $incid . "','" . $sid . "','" . $value . "','" . $field->fieldid . "', '-1')";
						$db->setQuery( $sql );
						$db->query(); //and create new corresponding to cid obtained during processing
						//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
					}
		...
		if ( strlen( $promo ) > 0 ) { //code was submitted
			$sql = "select * from #__digistore_promocodes where code='" . $promo . "' ";
			$db->setQuery( $sql );
			...
			if ( count( $promo_exists ) > 0 ) {//and there is such code in dabase
				...
				if ( ($promo_data->codeend >= $now || $promo_data->codeend == 0) && $promo_data->published == '1' && (($promo_data->codelimit - $promo_data->used) > 0 || $promo_data->codelimit == 0 ) && !($promo_data->forexisting != 0 && ($my->id < 1 || $licensecount < 1)) ) {
					$sql = "update #__digistore_session set cart_details='promocode=" . $promo . "' where sid='" . $sid . "'";
					//add this code to user's cart
				} else if ( $promo_data->published != '1' ) {
					$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODENP" )) . "' where sid='" . intval($sid) . "'";
				} else if ( $promo_data->codeend < $now && $promo_data->codeend != 0 ) {
					$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODEEXPIREDDATE" )) . "' where sid='" . intval($sid) . "'";
				} else if ( ($promo_data->codelimit - $promo_data->used) < 1 && $promo_data->codelimit != 0 ) {
					$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODEEXPIREDAMOUNT" )) . "' where sid='" . intval($sid) . "'";
				} else if ( $promo_data->forexisting != 0 && ($my->id < 1 || $licensecount < 1) ) {
					$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODEEXPIREDREGUSERONLY" )) . "' where sid='" . intval($sid) . "'";
				} else {
					$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODENA" )) . "' where sid='" . intval($sid) . "'";
				}
				...
				$sql = "update #__digistore_session set cart_details='promoerror=" . (JText::_( "DSPROMOCODEWRONG" )) . "' where sid='" . intval($sid) . "'";
				$db->setQuery( $sql );
				$db->query();
				//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
			}
		} else {//cleaning up promocode entry
			$sql = "update #__digistore_session set cart_details='' where sid='" . intval($sid) . "'";
			$db->setQuery( $sql );
			$db->query();
			//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
		}
		...
		if ( $shipto != '0' && intval( $shipto ) < 3 && intval( $shipto ) > 0 ) {
			$sql = "update #__digistore_session set shipping_details='" . $shipto . "' where sid='" . intval($sid) . "'";
			$db->setQuery( $sql );
			$db->query();
			//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
		} else {
			$sql = "update #__digistore_session set shipping_details='0' where sid='" . intval($sid) . "'";
			$db->setQuery( $sql );
			$db->query();
			//$jAp->enqueueMessage(nl2br($db->getErrorMsg()),'error');
		}

		//mosRedirect("index.php?option=com_digistore&task=checkout&Itemid=".$configs['itid'] );
	...
	function deleteFromCart( $customer, $configs )
	{
		...
		if ( (JRequest::getVar( 'discount', 0 ) == 0) && ($cid != -1) ) {
			...
			$sql = "select item_id, sid from #__digistore_cart where cid='" . intval($cid) . "'";

			$db->setQuery( $sql );
			$d = $db->loadObject();

			if ( isset( $d->sid ) && isset( $d->item_id ) ) {
				$sql = "delete from #__digistore_cartfields where sid='" . $d->sid . "' and productid='" . $d->item_id . "' and cid='" . intval($cid) . "'";
				$db->setQuery( $sql );
				$db->query();
			}

			$sql = "delete from #__digistore_cart where cid='" . intval($cid) . "'";
			...
			$sql = "update #__digistore_cart set quantity=quantity-" . $qty . " where cid='" . intval($cid) . "'";
	...
	function checkCartIsEmpty() {
		...
		$conf = $this->getInstance( "digistoreconfig", "digistoreModel" );
	...
	function emptyCart($sid = 0)
	{
		...
		$db = JFactory::getDBO();
		$sql = "delete from #__digistore_cartfields where sid='" . intval($sid) . "'";
		$db->setQuery( $sql );
		$db->query();

		$sql = "delete from #__digistore_cart where sid='" . intval($sid) . "'";
		$db->setQuery( $sql );
		$db->query();
	}
	...
	function getReturnUrl($type){
		...
		$success_url = $mosConfig_live_site . "/index.php?option=com_digistore&controller=" . $controller . "&task=" . $task . "&success=1&sid=" . $sid;
		$failed_url = $mosConfig_live_site . "/index.php?option=com_digistore&controller=" . $controller . "&task=" . $task . "&success=0&sid=" . $sid;
	...
	function checkSuccess( $sid )
	{
		...
		$db = JFactory::getDBO();
		$sql = "select count(sid) from #__digistore_cart where sid='" . intval($sid) . "'";
		$db->setQuery( $sql );
	...
	function proccessWait( $controller, $result )
	{
		...
		if ( $this->checkSuccess( $result['sid'] ) ) {
			...
			$return_url = "index.php?option=com_digistore&controller=digistoreCart&task=wait&processor={$processor}&pay=wait&sid={$sid}&count_redirect={$count_redirect}";
		}
	...
	function proccessSuccess($result, $pg_plugin, $order_id, $sid)
	{
		...
		{
			$db = JFactory::getDBO();
			$sql = "update #__digistore_settings
					set `in_trans`=1";
			$db->setQuery($sql);
			$db->query();
			$_SESSION['in_trans'] = 1;
		...
		if ($pg_plugin == 'authorizenet')
		{
			...
			if ($configs->afterpurchase == 0)
			{
				$return = JRoute::_(JURI::root()."index.php?option=com_digistore&view=digistorelicenses");
			}
			// orders page
			else
			{
				$return = JRoute::_(JURI::root()."index.php?option=com_digistore&view=digistoreorders");
			}
	...
	function proccessFail($controller, $result)
	{
		...
		if($result["processor"] == "payauthorize"){
			$return_url = "index.php?option=com_digistore&controller=digistoreCart&task=checkout";
	...
	function goToSuccessURL( $sid, $msg = '', $orderid = 0 )
	{
		...
		$mosConfig_live_site = digistoreHelper::getLiveSite(); //$jconf->live_site;
		...
		// Get return urls
		$success_url = /* $mosConfig_live_site. */JRoute::_( "index.php?option=com_digistore&controller=" . $controller . "&task=" . $task . "&success=1&sid=" . intval($sid) . '&Itemid=' . $_Itemid, false, false );
		$failed_url = /* $mosConfig_live_site. */JRoute::_( "index.php?option=com_digistore&controller=" . $controller . "&task=" . $task . "&success=0&sid=" . intval($sid) . '&Itemid=' . $_Itemid, false, false );
		...
		$content = '
			<form name="dsform" method="post" action="' . JRoute::_("index.php?option=com_digistore&controller=digistoreCart&task=showCart", false, false ) . '">
	...
	function loadCustomer($sid){
		$db = JFactory::getDBO();
		$sql = "select transaction_details from #__digistore_session where sid=" . intval($sid);
		$db->setQuery( $sql );
		$prof = $db->loadResult();
	...
	function storeTransactionData( $items, $orderid, $tax, $sid ){
		...
		$query = "select state, country, city from #__digistore_customers where id=" . $my->id;
		...
		$data['sid'] = $sid;
		$data['userid'] = $my->id;
		$data['option'] = 'com_digistore';
		$data['Itemid'] = $Itemid;

		$data['nontaxed'] = $tax['total'];
		$insert = base64_encode( serialize( $data ) );
		$sql = "update #__digistore_session set transaction_details='" . $insert . "' where sid='" . intval($sid) . "'";
...
//		$cust_info = $customer;
		...
		$sitename = (trim( $configs->store_name ) != '') ? $configs->store_name : $site_config->get( 'sitename' );
		$siteurl = (trim( $configs->store_url ) != '') ? $configs->store_url : $uri->base();
		...
		$query = "select lastname, company from #__digistore_customers where id=" . $my->id;
		$db->setQuery( $query );
		$customer_database = $db->loadAssocList();
		$lastname = $customer_database["0"]["lastname"];
		$copany = $customer_database["0"]["copany"];
...
//						<Your error code management>
				...
				$sql = "insert into #__digistore_logs(`userid`, `productid`, `emailname`, `to`, `subject`, `body`, `send_date`) values (".$my->id.", ".$item->id.", 'Product Email', '".$my->email."', '".addslashes(trim($subject))."', '".addslashes($message)."', '".$today."')";
				$db->setQuery($sql);
				$db->query();
				$sent[] = $item->name;
			}
	...
	function addOrder($items, $cust_info, $now, $paymethod, $status = "Active")
	{
		...
		if ( $shipto != '0' ) {
			if ( $shipto == '2' ) {
				$sql = "select shipaddress as a, shipzipcode as z, shipcity as c, shipstate as s, shipcountry as r from #__digistore_customers where id='" . $cust_info->_user->id . "'";
			} else if ( $shipto == '1' ) {
				$sql = "select address as a, zipcode as z, city as c, state as s, country as r from #__digistore_customers where id='" . $cust_info->_user->id . "'";
			} else {
				$sql = '';
			}
		}
		...
		$sql = "insert into #__digistore_orders ( userid, order_date, amount, amount_paid, currency, processor, number_of_licenses, status, tax, shipping, promocodeid, promocode, promocodediscount, shipto, fullshipto, published ) "
		...
		if ( $promoid > 0 ) {
			$sql = "update #__digistore_promocodes set `used`=`used`+1 where id=" . $promoid;
			$db->setQuery( $sql );
			$db->query();
		}
		return $orderid;
...
// start foreach
		foreach($items as $key=>$item)
		{
				...
				if(count($item->featured) <= 0)
				{
					for($i = 0; $i < $item->quantity; $i++)
					{
						...
						{// for renew
							...
							$sql = "update #__digistore_licenses
							...
							$sql = "insert into #__digistore_logs (`userid`, `productid`, `buy_date`, `buy_type`)
			...
			//for a new license
							...
							$sql = "insert into #__digistore_licenses(userid, productid, orderid, amount_paid, published, purchase_date, expires, plan_id, download_count, renew, renewlicid) "
						...
						$sql = "update #__digistore_products_emailreminders
								set `send`=0
						...
						if($item->usestock == '1'){
							$sql = "update #__digistore_products
									set used=used+1
...
// 						$lic_date->setOffset($jconfig->get('offset'));
						...
						if($item->renew == "0"){//new
							...
							$sql = "select `price`
									from #__digistore_products_renewals
									where `product_id`=".intval($item->featured[$i]->id)." and `default`=1";
						}
						$database->setQuery($sql);
						$database->query();
						...
						if(!isset($amount_paid)){// renew but not plan for renew
							$sql = "select `price` from #__digistore_products_plans where `product_id`=".intval($item->featured[$i]->id)." and `default`=1";
							$database->setQuery($sql);
							$database->query();
							$amount_paid = $database->loadResult();
						}
						...
						if($item->featured[$i]->planid != $unlimited_plan_id){ // not unlimited
							$sql = "select * from #__digistore_plans where id=".intval($item->featured[$i]->planid);
							$database->setQuery($sql);
							$database->query();
							$plan_values = $database->loadAssocList();
						...
						$sql = "insert into #__digistore_licenses (`userid`, `productid`, `orderid`, `amount_paid`, `published`, `ltype`, `package_id`, `purchase_date`, `expires`, `plan_id`, `download_count`, `renew`, `renewlicid`) "
...
// end foreach
		...
		foreach($license as $i => $v){
			if(!empty($v->productfields)){
				foreach($v->productfields as $i1 => $v1){
					...
					$sql = "insert into #__digistore_licensefields(licenseid, fieldname, optioname)
	...
	function addUserToList($user_id, $product_id){
		...
		$db = JFactory::getDBO();
		$sql = "select p.mailchimpapi, p.mailchimplist, p.mailchimpregister, p.mailchimpgroupid, u.email, c.firstname, c.lastname from #__users u, #__digistore_products p, #__digistore_customers c where u.id=c.id and u.id=".intval($user_id)." and p.id=".intval($product_id);
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadAssocList();
	...
	public function createLicense2( $order_id, $product, $user_id=null, $package_item=0, $published ){
		...
		$sql = 'INSERT INTO `#__digistore_licenses`
	...
	public function getSubProduct( $package_id ) {
		if( !isset( $this->packages[$package_id] ) ) {
			...
			$sql = 'SELECT  
					...
					FROM `#__digistore_featuredproducts` AS  `f` 
							INNER JOIN  
						`#__digistore_products` AS  `p` ON  `f`.`featuredid` =  `p`.`id` 
							LEFT JOIN  
						`#__digistore_products_plans` AS `pp` ON (`f`.`featuredid` =`pp`.`product_id` AND `f`.`planid` = `pp`.`plan_id` )
							LEFT JOIN 
						`#__digistore_plans` AS `pl` ON `f`.`planid` = `pl`.`id`
					WHERE 
	...
	public function getOrder( $order_id ){
		if(!isset( $this->orders[$order_id] )) {
			$db 	= JFactory::getDbo();
			$sql 	= 'SELECT * FROM `#__digistore_orders` WHERE `id`='.$order_id;
			$db->setQuery($sql);
			$order 	= $db->loadObject();
	...
	public function getNewLicenseId(){
		$db 	= JFactory::getDbo();
		$sql 	= "SELECT max(licenseid) FROM `#__digistore_licenses` WHERE CONCAT('',`licenseid`*1)=`licenseid`";
		$db->setQuery( $sql );
