...
class digistoreModeldigistorePlugin extends obModel
{
	var $_plugins;
	var $_plugin = null;
	var $_id = null;
	...
	function setId($id) {
		$this->_id = $id;
		$this->_installpath = JPATH_ROOT.DS."administrator".DS."components".DS."com_digistore".DS."plugins".DS;
		$this->_plugin = null;
	}
	...
	function getlistPlugins () {
		if (empty ($this->_plugins)) {
			$sql = "select * from #__digistore_plugins";
...
//		$plugin_path = $mosConfig_absolute_path.'/administrator/components/com_digistore/plugins/';
	...
	function getPlugin( $id = 0) {
		if (empty ($this->_plugin) || $id > 0) {
			if ($id > 0) $this->_plugin->id = $id;
			$this->_plugin = $this->getTable("digistorePlugin");
			$this->_plugin->load($this->_id);
			$this->_plugin->instance = $this->registerPlugin($this->_plugin->filename, $this->_plugin->classname);
//			$this->_plugin->instance = $this->registerPlugin($this->_plugin->filename, $this->_plugin->classname);
			$db = JFactory::getDBO();
			$sql = "select setting,description,value from #__digistore_plugin_settings where pluginid='".$this->_plugin->id."'";
			$db->setQuery($sql);
	...
	function getEncoders() {
		$db = JFactory::getDBO();
		$sql = "select id from #__digistore_plugins where type='encoding'";
		$db->setQuery($sql);
...
//		global $my, $digistor_enc_plugins, $mainframe, $dir, $r, $mosConfig_absolute_path;  
		...
		define( 'PCLZIP_TEMPORARY_DIR', JPATH_ROOT.DS.'administrator/components/digistore_product_uploads/tmp/' );
...
//		$this->recursive_remove_directory($tmpDir, TRUE);
	...
	*/
	function genLicenseFile ( $method, $zipFile, $subZipFile, $domain="localhost", $devdomain="localhost", $subdomains, $licenseFile, $passphrase, $trial_period = "", $outDir ) {
		...
		//@unlink( $mainframe->getCfg( 'absolute_path' ) . "/components/com_digistore/tmp/" . $my->id . "/$licenseFile" );
...
//	echo "Z";				 
		...
		$sql = "update #__digistore_session set transaction_details='".$ser_profile."' where sid=".$profile->_sid;
//echo $sql;
...
//		$this->record_transactiondata($plugin, "fail");
	...
	function addOrder ($items, $cust_info, $now, $paymethod, $status = "Active") {
		...
		$sql = "insert into #__digistore_orders ( userid,order_date,amount, currency, payment_method,number_of_licenses, status, tax, shipping, promocodeid, promocode, promocodediscount, shipto, fullshipto ) "
	...
	function addLicenses ($items, $orderid, $now , $customer, $status = "Active") {
		...
		$license = array();
		if ($status != "Pending") $published = 1; else $published = 0;
		$database = JFactory::getDBO();
		//lets add licenses to database
		$sql = "select licenseid from #__digistore_licenses";
		...
		$itemnum = count( $items);
		$license_index = 0;
		foreach ($items as $i => $v) if ($i<0) $itemnum--;
		for ($j = 0; $j < $itemnum; $j++) {
			...
			for ( $i=0;$i<$item->quantity;$i++) {
				...
				$sql = "insert into #__digistore_licenses ( licenseid, userid, productid, orderid, amount_paid, published) "
				...
				if ($item->usestock == '1') {
					$sql = "update #__digistore_products set used=used+1 where id = '".$item->item_id."'";
					$database->setQuery( $sql);
					$database->query();
				}

				if ($item->domainrequired == '3') {//if item is package - we have to update its type
					$sql = "update #__digistore_licenses set ltype='package' where id='".$license[$license_index]->id."'";
					$database->setQuery($sql);
					$database->query();
				}
			...
			for ( $i=0;$i<count($item->featured);$i++) {
				for ($k = 0; $k < $item->quantity; ++$k) {//inserting as many licenses as there are packages in the order
					...
					do {
						...
						//$sql = "select count(*) from #__digistore_licenses where licenseid=".$licenseid;
						//$database->setQuery($sql);
					...
					if ($licenseid) {
						$ls[] = $licenseid;
						$sql = "insert into #__digistore_licenses ( licenseid, userid, productid, orderid, amount_paid, published, ltype) "
								."values ('$licenseid','{$customer->_user->id}','{$item->featured[$i]->id}', '".$orderid."','0', ".$published.", 'package_item')";
						$database->setQuery( $sql);
						$database->query();

						$sql = "update #__digistore_products set used=used+1 where id = '".$item->featured[$i]->id."' and usestock='1'";
						$database->setQuery( $sql);
						$database->query();
		...
		foreach ($license as $i => $v) {
			if (!empty($v->productfields))
				foreach ($v->productfields as $i1 => $v1) {
					...
					$sql = "insert into #__digistore_licensefields(licenseid, fieldname, optioname) values ('".$v->licenseid."', '".$database->getEscaped($v1->name)."', '".$optionname."');";
					//Log::debug($sql);
					$database->setQuery($sql);
					$database->query();
					$sql = "";
...
//		$email = $this->getTable("digistoreMail");
		...
		foreach ($items as $i => $item) {
			if ($i <0) continue;
			if (!isset($counter[$item->id])) $counter[$item->id] = 1;
			$counter[$item->id]++;
		}
		foreach ($items as $i =>  $item) {
			if ($i <0) continue;
		...
		foreach ($items as $i => $item) {
			if ($i <0) continue;
			if (!in_array($item->name, $displayed)) $product_list .= $item->name.'<br />';
			$displayed[] = $item->name;
		}
...
//						<Your error code management>
	...
	function loadCustomer ($sid) {
		...
		$db = JFactory::getDBO();
		$sql = "select transaction_details from #__digistore_session where sid=".$sid;
		$db->setQuery($sql);
		$prof = $db->loadResult();
...
//		$data['session'] = $session;
		...
		$sql = "update #__digistore_session set transaction_details='".$insert."' where sid='".$sid."'";
		$database->setQuery($sql);
		$database->query();
	...
	function goToSuccessURL ($sid, $msg = '', $orderid = 0) {
		...
		if ( isset($cust_info) && is_array($cust_info) && isset($cust_info['cart']) ) {
			if ( isset($cust_info['cart']['total']) ) $cart_total = $cust_info['cart']['total'];
			if ( isset($cust_info['cart']['items']) ) $cart_items = unserialize($cust_info['cart']['items']);
			//debug($cart_items);
		}
		...
		$_Itemid = $Itemid;
		if (isset($customer->_Itemid) && ($customer->_Itemid > 0) ) $_Itemid = $customer->_Itemid;

		$cart = $this->getInstance("digistorecart", "digistoreModel");

		if ( isset($cart_items) ) { $items = $cart_items; }
		else { $items = $cart->getCartItems( $customer, $configs ); }
		...
		if ( $orderid == 0 && is_array($cust_info) && isset($cust_info['cart']) && isset($cust_info['cart']['orderid'])) $orderid = $cust_info['cart']['orderid'];
		if ( $orderid == 0 && is_object($cust_info) && isset($cust_info->cart['orderid'])) $orderid = $cust_info->cart['orderid']; // перестраховка если cart это об'ект
		...
		// Get return urls
		$success_url = /*$mosConfig_live_site.*/JRoute::_("index.php?option=com_digistore&controller=".$controller."&task=".$task."&success=1&sid=".$sid.'&Itemid='.$_Itemid,false,false);
		$failed_url = /*$mosConfig_live_site.*/JRoute::_("index.php?option=com_digistore&controller=".$controller."&task=".$task."&success=0&sid=".$sid.'&Itemid='.$_Itemid,false,false);
		...
		if (empty($msg) && $orderid > 0) $msg = JText::_("DSREFERENCEOID") . " " . $orderid;
		...
		$content = '
			<form name="dsform" method="post" action="'.JRoute::_("index.php?option=com_digistore&controller=digistoreCart&task=landingSuccessPage&Itemid=".$_Itemid,false,false).'">
	...
	function goToFailedURL ($sid, $msg = '') {
		...
		$content = '
			<form name="dsform" method="post" action="'.JRoute::_("index.php?option=com_digistore&controller=digistoreCart&task=landingFailedPage&Itemid=".$_Itemid).'">
				<input type="hidden" name="success_url" value="'.$success_url.'" />
				<input type="hidden" name="failed_url" value="'.$failed_url.'" />
				<input type="hidden" name="msg" value="'.$msg.'" />
				<input type="hidden" name="option" value="com_digistore" />
	...
	function performCheckout($customer) {
		...
		//lets check if user has digistore customer profile
		...
		if ((double )$total == 0) {
			...
			if ($content == "https") return $content;
			if ($content) {
				echo ($content);
				return;
			} else {
