...
class digistoreModeldigistoreOrder extends obModel
{
	var $_licenses;
	var $_license;
	var $_id = null;
		...
		if (empty ($this->_orders)) {
			...
			foreach ($this->_orders as $i => $v) {
				...
				$sql = "SELECT dp.id, dp.name, dl.licenseid, dp.hide_public
						FROM #__digistore_products dp
						INNER JOIN #__digistore_licenses dl ON ( dl.productid = dp.id )
				...
				$sql = "SELECT dp.id, dp.name, GROUP_CONCAT(dl.licenseid) as licenseid, dp.hide_public
						FROM #__digistore_products dp
						INNER JOIN #__digistore_licenses dl ON (dl.package_id = dp.id)
				...
				if(is_array($this->_orders[$i]->products)){
					foreach ($this->_orders[$i]->products as $j => $p) {
						$sql = "SELECT catid FROM #__digistore_product_categories WHERE productid=".$p->id;
						$db->setQuery($sql);
	...
	function getOrder($id = 0) {
		if (empty ($this->_order)) {
			...
			/*$sql = "select p.*, l.amount_paid as price, l.licenseid, l.id as lid, l.amount_paid from #__digistore_products p, #__digistore_licenses l "
			...
			if(isset($prods2)){
				foreach($prods2 as $key=>$product){
					$sql = "SELECT p.price AS total FROM #__digistore_licenses l, #__digistore_products_plans p where l.`orderid`=".intval($id)." and l.package_id=".intval($product->id)." and l.package_id=p.product_id";
					$db->setQuery($sql);
					$db->query();
