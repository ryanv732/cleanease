...
class digistoreModeldigistoreCategory extends obModel
{
	...
	function __construct () {
		...
		$limit = $configs->catlayoutcol * $configs->catlayoutrow ;//$mainframe->getUserStateFromRequest( 'global.list.limit', 'limit', $mainframe->getCfg('list_limit'), 'int' );
	...
	function getlistCategories(){
		...
		if(empty ($this->_categories)){
			...
			$where[] = "(
							...
							or
							(select count(cc.id)
							 from #__digistore_categories cc
							 where cc.parent_id=c.id and cc.published=1) > 0
						) ";

			$sql = "select c.* from #__digistore_categories c ".(count($where) > 0? " where ": "") . implode (" and ", $where);
		...
		if(isset($this->_categories) && count($this->_categories) > 0){
			...
			foreach($this->_categories as $key=>$category){
				if(trim($category->images) != ""){
					...
					copy(JPATH_SITE.$category->images, JPATH_SITE.DS."images".DS."stories".DS."digistore".DS."categories".DS.$images);
					$this->_categories[$key]->image = $images;
					$this->_categories[$key]->images = "";
					$sql = "update #__digistore_categories set `image`='".trim($images)."', `images` = '' where id=".intval($category->id);
					$db->setQuery($sql);
					$db->query();
				}
			}
	...
	function getCategoryProducts($id = 0) {
		...
		$sql = "select id
				...
				where id in (select productid from #__digistore_product_categories where catid='".intval($id)."')
				  and hide_public=0".
			(count($where) > 0? " and ": "") .
			implode (" and ", $where);
		;
	...
	function getConfigs(){
		$db = JFactory::getDBO();
		$sql = "select * from #__digistore_settings";
		$db->setquery($sql);
		$db->query();
	...
	function __showCategoriesItem($level, $cat, & $cats, & $output)
	{
		...
		if ( $cat->catcount > 0 ) {
			$catlink = JRoute::_("index.php?option=com_digistore&controller=digistoreCategories&task=view&cid=".$cat->id."&Itemid=".$Itemid);
		} else {
			$catlink = JRoute::_("index.php?option=com_digistore&controller=digistoreProducts&task=list&cid=".$cat->id."&Itemid=".$Itemid);
		}
	...
	function getCategoriesTreeIterator( $id,  $level, & $cats ) {
		...
		foreach( $cats as $key => $tcat ) {
			...
			if ($tcat->parent_id == $id) {
				...
				$sql = "select count(*) from #__digistore_categories c where parent_id=".$tcat->id;
				$this->_db->setQuery($sql);
				$cat->catcount = $this->_db->loadResult();

				$sql = "select count(*) from #__digistore_products p 
						inner join #__digistore_product_categories c on (p.id = c.productid) 
	...
	function getCategoryCategories($id = 0) {
		...
		if (empty ($this->_categorycats)) {
			...
			$where[] = " ((select count(p.id) from #__digistore_products p where 
					p.hide_public = 0 and
					p.id IN (
							SELECT productid
							FROM #__digistore_product_categories
					...
					) > 0 or (select count(cc.id) from #__digistore_categories cc where cc.parent_id=c.id and cc.published=1) > 0 )";

			$sql = "select c.* from #__digistore_categories c where parent_id='".(($id < 1)? $this->_category->id:$id)."'".
				(count($where) > 0? " and ": "") .
				implode (" and ", $where);
				;
			...
			$this->_total = @$this->_getListCount($sql);
			$order = " order by ordering asc ";
			$x = $this->_getList($sql.$order, $this->getState('limitstart'), $this->getState('limit'));
