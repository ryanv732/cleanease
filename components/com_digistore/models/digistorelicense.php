...
class digistoreModeldigistoreLicense extends obModel
{
	var $_licenses;
	var $_license;
	var $_id = null;
	...
	function incrimentDownloadCount($licid) {
		$db = JFactory::getDBO();
		$sql = 'update #__digistore_licenses set download_count = download_count+1 where id='.intval($licid);
		$db->setQuery($sql);
		$db->query();
	}
	...
	function getlistLicenses(){
		$user = new digistoreSessionHelper();
		$db = JFactory::getDBO();
		...
		if (empty ($this->_licenses)) {
			...
			." from #__digistore_licenses l, #__digistore_products p, #__users u, #__digistore_orders o "
			." where l.productid=p.id and l.userid=u.id and l.userid=".$user->_user->id;
			$sql .= " and l.published=1 and l.orderid=o.id ". ($this->_oid > 0? (" and l.orderid=".$this->_oid." ") : "");
			...
			$sql = "select id from #__digistore_products where `domainrequired`=3";
			$db->setQuery($sql);
			$db->query();
			...
			." from #__digistore_licenses l, #__digistore_products p, #__users u, #__digistore_orders o "
			." where l.productid=p.id and l.userid=u.id and l.userid=".$user->_user->id;
			$sql .= " and l.published=1 and l.orderid=o.id and l.productid not in (".implode(",", $packages_id).") ". ($this->_oid > 0? (" and l.orderid=".$this->_oid." ") : "");
			...
			if(isset($this->_licenses)){
				foreach ($this->_licenses as $i => $v) {
					$sql = "select catid from #__digistore_product_categories where productid=".$v->productid;
					$db->setQuery($sql);
		...
		if(isset($this->_licenses)){
			foreach ($this->_licenses as $i => $license){
					$sql = "select fieldname, optioname from #__digistore_licensefields where licenseid='".$license->id."'";
					$db->setQuery($sql);
		...
		if(isset($this->_licenses)){
			foreach ($this->_licenses as $i => $license){
				$sql = "select p.id, pp.product_id, pp.price, p.name, p.duration_count, p.duration_type  from #__digistore_products_plans pp
						left join #__digistore_plans p on (p.id = pp.plan_id) where pp.product_id = ".$license->productid;
				$db->setQuery($sql);
		...
		if(isset($this->_licenses)){ 
			foreach ($this->_licenses as $i => $license){
				$sql = "select p.id, pp.product_id, p.name, p.duration_count, p.duration_type from #__digistore_products_renewals pp
						left join #__digistore_plans p on (p.id = pp.plan_id) where pp.product_id = ".$license->productid;			
				$db->setQuery($sql);
	...
	function getLicense($id = 0) {
		if (empty ($this->_license)) {
			if ($id > 0) $this->_id = $id;
			//$sql = "select l.*,p.name as productname, u.username from #__digistore_licenses l, #__digistore_products p, #__users u where l.productid=p.id and l.userid=u.id and l.id=".$this->_id;
			$db = JFactory::getDBO();
			$sql = "select l.plan_id from #__digistore_licenses l where l.id=".intval($this->_id);
			$db->setQuery($sql);
			$db->query();
			...
			$sql = "select l.*,p.name as productname, u.username, pl.name as planname,	pl.duration_count, pl.duration_type
				from #__digistore_licenses l, #__digistore_products p, #__users u, #__digistore_plans pl
...
//			$this->setRedirect("index.php?option=com_digistore&controller=digistoreLicenses" );
		...
		$sql = "update #__digistore_licenses set dev_domain='".$dev."', domain='".$prod."', domain_change='".$license->domain_change."' where id=".$license->id;
		$db->setQuery($sql);
		$db->query();
	...
	function prepareDownload($product, $uid) {
		...
		$usermainDir = JPATH_ROOT.DS."components/com_digistore/download/";
		...
		digistoreHelper::CreateIndexFile($usermainDir);
		digistoreHelper::CreateIndexFile($prepDir);		
	...
	function getPackageContents($license, $product) {
		...
		digistoreHelper::CreateIndexFile(JPATH_ROOT.DS.'administrator/components/digistore_product_uploads/');
		digistoreHelper::CreateIndexFile(JPATH_ROOT.DS."components/com_digistore/download/");
		digistoreHelper::CreateIndexFile(JPATH_ROOT.DS."components/com_digistore/download/".$license->userid."/");

		require_once (JPATH_ROOT.DS."administrator".DS."includes".DS."pcl".DS."pclzip.lib.php");

		$srcZip = new PclZip(JPATH_ROOT.DS."components/com_digistore/download/".$license->userid."/".$product->file);
	...
	function getNrOrders($lic_id){
		$db = JFactory::getDBO();
		$count = "0";
		$sql = "select `orderid`, `old_orders` from #__digistore_licenses where `licenseid` = '".$lic_id."'";
		$db->setQuery($sql);
		$db->query();
