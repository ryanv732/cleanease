...
function digistoreBuildRoute(&$query){
	global $option;
	$segments = array();
	$db = JFactory::getDBO();
	...
	if(isset($query['controller'])){
		$segments[] = str_replace('digistore', '', strtolower($query['controller']));
		unset($query['controller']);
	}
	else{
		if(isset($query['view'])){
			$segments[] = str_replace('digistore', '', strtolower($query['view']));
			unset($query['view']);
		}
		else{
			if(isset($menuItem) && isset($menuItem->query)){
				$segments[] = str_replace('digistore', '', strtolower($menuItem->query['view']));
			}
		}
	}
	...
	if(isset($query['returnpage'])){
		$segments[] = str_replace('digistore', 'return_to_', strtolower($query['returnpage']));
...
function digistoreParseRoute($segments){
	$vars = array();
	$db = JFactory::getDBO();
	$app = JFactory::getApplication();
	$menu = $app->getMenu();
	...
	$controller = isset($segments['0']) && intval($segments['0']) == "0" ? 'digistore'.ucfirst($segments['0']) : null;
	$vars['controller'] = $controller;
	//Handle View and Identifier

	if(!isset($controller)){
		$controller = "digistoreProducts";
		$vars['controller'] = $controller;
		array_unshift($segments, $controller);
	}

	switch($controller){
		case 'digistoreProducts' :
			if(isset($segments['1'])){
				...
				elseif(isset($segments[1])){
					...
					switch($vars['task']){
						...
						case 'login':
							...
							$vars['returnpage'] = 'digistore'.ucfirst($vars['returnpage']);
							break;
