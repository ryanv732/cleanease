...
class digistoreVideo{

	public static function createVideo(){
		$db = JFactory::getDBO();
		$product_id = JRequest::getVar("pid", "0");
		$sql = "select `video_url`, `video_width`, `video_height` from #__digistore_products where id=".intval($product_id);
		$db->setQuery($sql);
		$db->query();
		...
		if(trim($product["0"]["video_url"]) != ""){
			...
			if(intval($video_width) != 0 && intval($video_height) != 0){
				...
				$media = digistoreVideo::jwAllVideos( $media, $video_width, $video_height, $video_width, $video_height);
			}//if height and width
		}//if url
		return $media;
	}
			...
			*/
			...
			if(!$loadJWAVcss) {
				...
				<style type="text/css" media="all">
					@import "'.$mosConfig_live_site.'/plugins/content/jw_allvideos/jw_allvideos/tmpl/Classic/css/template.css";
				</style>
				';

				if($av_compressjs){
				$jwavhead .= '
		<script type="text/javascript" src="'.$mosConfig_live_site.'/plugins/content/jw_allvideos/jw_allvideos/players/jw_allvideos_scripts.php"></script>
				';
				}
				else {
					// Assign paths
					...
					if(version_compare(JVERSION,'1.6.0','ge')) {
						$pluginLivePath = JURI::base(true).'/plugins/content/jw_allvideos/'.$plg_name.'/'.$plg_name;
					} else {
						$pluginLivePath = JURI::base(true).'/plugins/content/jw_allvideos/'.$plg_name;
					}

					if($gzipScripts){
						$document->addScript($pluginLivePath.'/includes/js/jw_allvideos/jw_allvideos.js.php');
					} else {
