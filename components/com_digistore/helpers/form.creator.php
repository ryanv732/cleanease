...
if (!class_exists("formCreator")) {
	...
	class formCreator {
		...
		function formCreator($form_name, $action, $form_id='') {
			...
			$this->setImgPath(JURI::root().'components/com_digistore/assets/images/' );
			if (empty($form_id)) {
...
<tr{$style_row}>
	<td>
		<input type='button' class='digistore_cancel' onclick='history.go(-1);' value='	Back	' />&nbsp;
		<input type='submit' name='submitBtn' class='digistore_continue' value='{$params['value']}' size='30' maxsize='32'>
	</td>
	<td>&nbsp;
	...
	function checkExpire(month, year){
		...
		if( !CheckDateValue ){
			...
			if (true == $this->autoSubmit){
				...
				$form .= digistoreHelper::getSubmitForm($configs, 'paypaypal');
