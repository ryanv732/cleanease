	...
	if (isset($products_id) && count($products_id) > 0)
	{
		foreach ($products_id as $key=>$product_id)
		{
			...
			if(isset($result) && trim($result["0"]["productemailsubject"]) != "" && trim($result["0"]["productemail"]) != "")
			{
				...
				$sql = "select u.*, c.*
						from #__users u, #__digistore_customers c
...
// 				JUtility::sendMail($from, $fromname, $recipient, $subject, $body, $mode);
				...
				$sql = "insert into #__digistore_logs(`userid`, `productid`, `emailname`, `to`, `subject`, `body`, `send_date`)
		...
		$sql = "select `subject`, `body` from #__digistore_mailtemplates where `type`='order'";
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadAssocList();
...
function getProductEmails($product_id)
{
	$db = JFactory::getDBO();
	$sql = "select per.*, er.*
			from #__digistore_products_emailreminders per, #__digistore_emailreminders er
...
{
	$db = JFactory::getDBO();
	$sql = "select * from #__digistore_plans";
	$db->setQuery($sql);
	$db->query();
...
function processDigiText($text, $email, $license_value, $user_details, $plan)
{
	...
	$product_url = JURI::root()."index.php?option=com_digistore&controller=digistoreProducts&task=view&pid=".intval($email["product_id"])."&cid=".intval($category_id);
	$product_url = '<a href="'.$product_url.'">'.$product_url.'</a>';
	$renew_url = JURI::root()."index.php?option=com_digistore&controller=digistoreCart&task=add&pid=".intval($email["product_id"])."&cid=".intval($category_id);
	$renew_url = '<a href="'.$renew_url.'">'.$renew_url.'</a>';
	$subscription_term = $plan["name"];
	$license_number = $license_value["licenseid"];
	$my_licenses = JURI::root()."index.php?option=com_digistore&view=digistorelicenses";
	...
	$my_orders = JURI::root()."index.php?option=com_digistore&controller=digistoreOrders&task=list";
	$my_orders = '<a href="'.$my_orders.'">'.$my_orders.'</a>';

	$sql = "select currency from #__digistore_settings";
	$db->setQuery($sql);
	$db->query();
	...
	$sql = "select dp.name, dpr.price
			from #__digistore_plans dp, #__digistore_products_renewals dpr
...
function sendEmail($email, $license_value, $plan)
{
	...
	if(intval($result) == "0"){
		...
		$sql = "insert into #__digistore_logs(`userid`, `productid`, `emailname`, `emailid`, `to`, `subject`, `body`, `send_date`)
...
function cronjobs()
{
	...
	if($day_today != $day_last_check)
	{
		...
		require_once(JPATH_SITE.DS."administrator".DS."components".DS."com_digistore".DS."helpers".DS."helper.php");
		...
		foreach($rows as $row) {
			digistoreAdminHelper::expireUserProduct($row->userid);
		}
	} // If today not search
} // End function
