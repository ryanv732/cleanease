...
if($prod->published == 0){//if product is unpublished
	echo '<span class="redirect_message">'.JText::_("DIGI_PRODUCT_UNPUBLISHED")." ".'<a href="'.JRoute::_("index.php?option=com_digistore&controller=digistoreCategories&Itemid=".$Itemid).'">'.JText::_("DIGI_HOME_PAGE").'</a></span>';
	return true;
}
...
if (!$prod->id)
{
	echo "<div style='padding:20px;text-align:center;'>".JText::_("DSPRODNOTAVAILABLE")."<br/><br/><a href='".JRoute::_("index.php?option=com_digistore")."'>".JText::_('DSCONTINUESHOPING')."</a></div>";
	return;
}
...
$price =  $prod->price;
$i = $this->i;
$validation_js_script = digistoreHelper::addValidation($prod->productfields, $i);
$doc = JFactory::getDocument();
$doc->addScriptDeclaration( $validation_js_script );
// $doc->addScript(JURI::root()."components/com_digistore/assets/js/SqueezeBox.js");
$doc->addScript(JURI::root()."components/com_digistore/assets/js/createpopup.js");
// $doc->addStyleSheet(JURI::root()."components/com_digistore/assets/css/SqueezeBox.css");
...
<script language="javascript" type="text/javascript">
	function displayImage(key, type){
		...
		var url = 'index.php?option=com_digistore&controller=digistoreProducts&task=getimage&type='+type+'&format=raw&position='+key+'&pid='+<?php echo intval($prod->id); ?>;
...
<form name="prod" id="product-form" action="index.php" method="post" style="width:100%;" onsubmit="return prodformsubmitA<?php echo $i; ?>()">
<?php endif; ?>
	...
	<div class="ijd-box ijd-rounded row-fluid">
			...
			<div id="total_slide">
				...
				<div id="slide" style="<?php echo $style; ?>">
					...
					<?php
						if(isset($prodimages["0"])){
					?>
							<a onclick="javascript:grayBoxiJoomla('index.php?option=com_digistore&controller=digistoreProducts&task=previwimage&tmpl=component&position=0&pid=<?php echo intval($prod->id); ?>', <?php echo $gray_size_w; ?>, <?php echo $gray_size_h; ?>)">
				...
				<?php
					if(count($prodimages) > 1){
						if($conf->gallery_style == 0){
							echo digistoreHelper::getGalleryScroller($prod, $prodimages, $conf);
						}
						else{
							echo digistoreHelper::getGallerySimple($prod, $prodimages, $conf);
						}
					}
				?>
		...
		<?php endif; ?>
			...
			<?php
				require_once(JPATH_COMPONENT_SITE.DS.'helpers'.DS.'video.php');
				echo digistoreVideo::createVideo();
			?>
							...
							elseif(($prod->usestock == 1) && ($prod->emptystockact == 1) && ($prod->used >= $prod->stock)){
								...
								else
								{
									if($conf->afteradditem == "2")
									{
										...
										<button type="button" class="btn btn-warning" onclick="javascript:createPopUp(<?php echo $prod->id; ?>, <?php echo JRequest::getVar("cid", "0"); ?>, '<?php echo JURI::root(); ?>', '', '', <?php echo $cart_itemid; ?>, '<?php echo JRoute::_("index.php?option=com_digistore&view=digistorecart&Itemid=".$cart_itemid); ?>');"><i class="ico-shopping-cart"></i> <?php echo JText::_("DSADDTOCART");?></button><?php
									}
									else
									{ ?>
