...
<div class="bar">
	...
	<table id="digi_table" cellspacing="0" <?php echo $table_style; ?>>
		<tr>
			...
			?>
			<th <?php if($configs->showcremove == 0){ echo $invisible; $nr_columns --;}?> >
			</th>
	...
	?>
		<tr class="item_row">
			...
			<td class="item_column">
				<ul>
					<li class="digistore_product_name"><?php echo $item->name.$renew; ?></li>
				</ul>
			</td>
			<!-- /End Product name -->
			...
			<!-- Quantity -->
			<td align="center" <?php if ($configs->showcam == 0) echo $invisible;?> nowrap="nowrap" class="item_column">
				<ul>
					<li class="digistore_product_name">
			...
			<td style="display:none;" nowrap="nowrap" class="item_column">
				<span class="digi_cart_amount" id="cart_item_price<?php echo $item->cid; ?>"><?php echo digistoreHelper::format_price($item->price, $item->currency, true, $configs); ?></span>
			</td>
			<!-- /End Price -->
			...
			<td class="item_column" nowrap="nowrap" <?php echo ($shippingexists >0 ) ? "style='text-align:center;'" : 'style="text-align:center; display:none;"';?>><span class="digi_cart_amount"><?php
					...
					if($item->domainrequired == 2){
						$shipping_value = (isset($item->shipping) && $item->domainrequired==2 ? digistoreHelper::format_price($item->shipping, $item->currency, true, $configs) : "N/A");
			...
			<!-- Remove -->
			<td class="item_column" style="text-align:center; <?php if ($configs->showcremove == 0) echo "display:none;";?>" nowrap="nowrap">
				<?php
					$remove_link = "index.php?option=com_digistore&controller=digistoreCart&task=deleteFromCart&cartid=".$item->cid . (isset($item->discount1)?('&discount=1&noupdate='.(isset($item->noupdate)?$item->noupdate:'').'&qty='.$item->quantity ):"" )."&Itemid=".$Itemid;
				?>
				<!-- <input type="button" class="digi_remove_button" value="" onClick="window.location='<?php echo JRoute::_($remove_link); ?>';"> -->
				<img height="25" src="<?php echo JURI::root()."components/com_digistore/assets/images/icon_trash.png"; ?>" onClick="window.location='<?php echo JRoute::_($remove_link); ?>';">
			</td>
			<!-- /End Remove -->
		...
		<tr>
			...
			<td nowrap="nowrap" style="text-align: center; <?php echo $border_bottom; ?> padding-top:15px;">
				...
				<ul style="margin: 0; padding: 0;">
					...
					<?php  if (($tax['value'] > 0) || ($configs->tax_zero == 1) && ($customer->_user->id > 0)) : ?>
			...
			<td nowrap="nowrap" style="text-align: center; <?php echo $border_bottom; ?> padding-top:15px;">
				<ul  style="margin: 0; padding: 0;" >
					...
					<?php if ($tax['promo'] > 0 && $tax['promoaftertax'] == '0') : ?>
					<li class="digi_cart_amount"><?php echo digistoreHelper::format_price($tax['promo'], $tax['currency'], true, $configs) ?></li>
					<?php endif;?>

					<?php if (($tax['value'] > 0 || $configs->tax_zero == 1) && $customer->_user->id > 0) : ?>
					<li class="digi_cart_amount"><?php echo digistoreHelper::format_price($tax['value'], $tax['currency'], true, $configs); ?></li>
					<?php endif; ?>

					<?php if ($tax['shipping'] > 0 && $customer->_user->id > 0) : ?>
					<li class="digi_cart_amount"><?php echo digistoreHelper::format_price($tax['shipping'], $tax['currency'], true, $configs); ?></li>
					<?php endif; ?>
		...
		<tr>
			<td colspan="<?php echo $nr_columns - 1; ?>">
				<table>
					<tr>
						<td>
							<span class="digistore_details"><?php echo JText::_("DSPROMO"); ?>:</span>
						</td>
						<td>
			...
			<td nowrap="nowrap" style="text-align: center;">
				<ul style="margin: 0; padding: 0;">
					<li class="digi_cart_amount" id="cart_total" style="color:green;"><?php echo digistoreHelper::format_price($tax['taxed'], $tax['currency'], true, $configs); ?></li>
				</ul>
			</td>
		</tr>
		<?php $k++; ?>
					...
					<td colspan="6" style="padding-top:10px;">
						<table width="100%" cellspacing="0">
							<td width="50%" align="left" style="padding-left:10px;">
								<input type="button" class="digistore_cancel" name="continue" value="<?php echo JText::_("DSCONTINUESHOPING")?>" 
									   onClick="window.location='<?php echo digistoreHelper::DisplayContinueUrl($configs, $cat_url); ?>';" />
							</td>
