...
*/
	...
	$categories_005 = $this->getTableData('#__digistore_categories','','id, images, image, thumb', 'objectlist');
	...
	foreach ( $categories_005 as $cat ) {
		...
		$this->updateTable('#__digistore_categories', $result_images, ' id='.$cat->id );
	}
...
function convertCategoriesImages_005( $cat ) {
		...
		if ( !empty($pathinfo_005) ) {
			 ...
			 $categories_path = $categories_rel_path . 'images' . DS . 'stories' . DS . 'digistore' . DS . 'categories' . DS;
