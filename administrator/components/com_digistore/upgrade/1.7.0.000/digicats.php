...
*/
	...
	$sql = "select id, name, parent_id, published from #__digistore_categories";
	$this->db->setQuery($sql);
	...
	if (!$digitype) {
		$sql = "insert into #__menu_types (`menutype`, `title`, `description`) values ('digicats', 'DigiStore Categories', 'Digistore Category links')";
		$db->setQuery($sql);
		$db->query();
	}
	...
	foreach ($catids as $i => $v) {
		...
		$sibs = $db->loadResult();
		if ($sibs > 0) {
			$link = 'index.php?option=com_digistore&controller=digistoreCategories&task=view&cid[]='.$v->id; 
		} else {
		...
		if (!$parent_id) {
			$parent_link = 'index.php?option=com_digistore&controller=digistoreCategories&task=view&cid[]='.$v->parent_id;
			$sql = "select id from #__menu where menutype='digicats' and link='".$parent_link."'";
			$db->setQuery($sql);
			$parent_id = $db->loadResult();
		}
