...
class digistoreAdminModeldigistoreTaxrule extends JModelList {
	protected $_context = 'com_digistore.digistoreTaxrule';
	...
	protected function getListQuery(){
		...
		$sql = "select * from #__digistore_tax_rule";
		return $sql;
	}
	...
	function getRule() {
		if (empty ($this->_rule)) {
			$this->_rule = $this->getTable("digistoreTaxrule");
	...
	function store () {
		$item = $this->getTable('digistoreTaxrule');
...
//			$this->setError($item->getErrorMsg());
	...
	function publish () {
		...
		$item = $this->getTable('digistoreTaxrate');
		$res = 0;
		if ($task == 'publish'){
			$res = 1;
			$sql = "update #__digistore_tax_rule set published='1' where id in ('".implode("','", $cids)."')";
		} else {
			$res = -1;
			$sql = "update #__digistore_tax_rule set published='0' where id in ('".implode("','", $cids)."')";
...
//		return false;
	...
	function shiftorder($direction = 1) {
		...
		$sql = "update #__digistore_tax_rule set `ordering`=ordering".($direction == 1?"+1":"-1")." where id=".$cids[0];
		$db->setQuery($sql);
		$res = $db->query();
		if (!$res) return $res;
		return true;
	...
	function reorder () {
		...
		$order = JRequest::getVar('order', array(0), 'post', 'array');
		foreach ($cids as $i => $v) {
			$sql = "update #__digistore_tax_rule set `ordering`='".$order[$i]."' where id=".$v;
			$db->setQuery($sql);
			$res = $db->query();
			if (!$res) return $res;
		}
	...
	function getlistProductTaxClasses () {
		$sql = "select * from #__digistore_tax_productclass";
		$db = JFactory::getDBO();
		$db->setQuery($sql);
		return ($db->loadObjectList());
	}
	...
	function getlistTaxRates () {
		$sql = "select * from #__digistore_tax_rate";
		$db = JFactory::getDBO();
		$db->setQuery($sql);
		return ($db->loadObjectList());
	}
	...
	function getlistProductClasses () {
		$sql = "select * from #__digistore_productclass";
		$db = JFactory::getDBO();
		$db->setQuery($sql);
		return ($db->loadObjectList());
