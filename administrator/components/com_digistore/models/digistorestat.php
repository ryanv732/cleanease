...
class digistoreAdminModeldigistoreStat extends obModel
{
	var $_configs = null;
	var $_id = null;
	...
	function getPaginationDate($configs){
		...
		if(trim($action) == ""){
			if($report == "daily"){
				echo "(".date($configs->time_format).")";
			}
			elseif($report == "weekly"){
				$today = strtotime(date("Y-m-d"));
				$date = $this->getStartEndDate($report);
				echo "(".date($configs->time_format, strtotime($date["0"]))." ".JText::_("DSTO")." ".date($configs->time_format, strtotime($date["1"])).")";
			}
		}
		elseif($action != ""){
			if($report == "daily"){
				...
				echo date($configs->time_format, $today);
			}
			elseif($report == "weekly"){
				$date = $this->getStartEndDate($report);
				$start_date = strtotime($date["0"]);
				$end_date = strtotime($date["1"]);
				echo "(".date($configs->time_format, $start_date)." ".JText::_("DSTO")." ".date($configs->time_format, $end_date).")";
			}
	...
	function getStartEndDate($report){
		...
		elseif($report == "yearly"){
			$sql = "select min(order_date) as min_date, max(order_date) as max_date from #__digistore_orders";
			$db->setQuery($sql);
			$db->query();
