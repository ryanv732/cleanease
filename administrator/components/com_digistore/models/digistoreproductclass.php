...
class digistoreAdminModeldigistoreProductclass extends JModelList {

	protected $_context = 'com_digistore.digistoreProductclass';
	var $_pclasses;
	var $_pclass;
	var $_id = null;
	var $_total = 0;
	...
	protected function getListQuery() {
		...
		$query->select('*');
		$query->from('#__digistore_productclass');
		$query->where($where);
		return $query;
	}
	...
	function getproductClass() {
		if (empty ($this->_pclass)) {
			$this->_pclass = $this->getTable("digistoreProductClass");
	...
	function store () {
		$item = $this->getTable('digistoreProductClass');
		...
		if(trim($data["ordering"]) == "" || trim($data["ordering"]) == "0"){
			$db = JFactory::getDBO();
			$sql = "select max(`ordering`) from #__digistore_productclass";
			$db->setQuery($sql);
			$db->query();
	...
	function publish () {
		...
		if ($task == 'publish'){
			$res = 1;
			$sql = "update #__digistore_productclass set published='1' where id in ('".implode("','", $cids)."')";
		} else {
			$res = -1;
			$sql = "update #__digistore_productclass set published='0' where id in ('".implode("','", $cids)."')";
...
//		return false;
	...
	function shiftorder($direction = 1) {
		...
		$sql = "update #__digistore_productclass set `ordering`=ordering".($direction == 1?"+1":"-1")." where id=".$cids[0];
		$db->setQuery($sql);
		$res = $db->query();
		if (!$res) return $res;
		return true;
	...
	function reorder () {
		...
		$order = JRequest::getVar('order', array(0), 'post', 'array');
		foreach ($cids as $i => $v) {
			$sql = "update #__digistore_productclass set `ordering`='".$order[$i]."' where id=".$v;
			$db->setQuery($sql);
			$res = $db->query();
			if (!$res) return $res;
		}
