...
class digistoreAdminModeldigistoreLanguage extends obModel
{
	...
	function getlistLanguages () {
		if (empty ($this->_plugins)) {
			$sql = "select * from #__digistore_languages";
		...
		while ($f = readdir($dir)) {
			if ($f != "." && $f != ".." && is_dir($dirname.$f)) {
				...
				while ($ff = readdir($subdir)) {
					if (strpos($ff, "mod_digistore") !== false) {
//						echo $ff."<br />";
						$x = new stdClass;
						$x->id = 0;
						$x->name = $f;
	...
	function getLanguage () {
		...
		if (intval($this->_id) > 0) {
			$db = JFactory::getDBO();
			$sql = "select * from #__digistore_languages where id=".$this->_id;
			$db->setQuery($sql);
...
//			$task = JRequest::getVar("task", "", "request");
	...
	function store () {
		...
		if (intval($id) < 1) {
			...
			$sql = "select * from #__digistore_languages where id=".$id;
			$db->setQuery($sql);
			$lang = $db->loadObjectList();
			$lang = $lang[0];
			$file = $lang->fefilename;
...
//		@copy($path, $respath.$menufile);
	...
	function installLanguage($path, $language_file = '') {
		...
		foreach ($lang_code as $code ) {
			...
			$fe_path = $path."fe".DS.$code.DS.$code.".com_digistore.ini";
			$lang_file = $code.".com_digistore.ini";
			$be_path = $path."be".DS.$code.DS.$code.".com_digistore.ini";
			if (!file_exists($fe_path)) $fe = 0; else $fe = 1;
			if (!file_exists($be_path)) $be = 0; else $be = 1;
//			echo $be." ".$be_path." ".$fe." ".$fe_path;die;
			if ($be && $fe ) {
				$query = "select count(*) from #__digistore_languages where fefilename='".$lang_file."' or befilename='".$lang_file."'";
				$db->setQuery($query);
...
//die;
	...
	function upload() {
		...
		$install_path = JPATH_ROOT.DS."tmp".DS."digistorelanguage".DS;
	...
	function delete () {
		...
		foreach ($cids as $cid) {
			$sql = "select name,fefilename from #__digistore_languages where id=".$cid;
			$db->setQuery($sql);
			...
			if ((JFile::delete($respathfe.$file)) && (JFile::delete($respathfe.$menufile))
				...
				$sql = "delete from #__digistore_languages where id=".$cid;
				$db->setQuery($sql);
				$db->query();
			}
		}
