...
<script type="text/javascript">
	function changeCategory(category_id){
		var url = "index.php?option=com_digistore&controller=digistoreLicenses&task=changeCategory&category_id="+category_id+"&format=raw&tmpl=component";
...
<form id="adminForm" action="index.php" name="adminForm" method="post">
	...
	<div style="text-align:right">
		...
		<div style="padding-bottom:0.5em">
			<?php echo JText::_("DSKEYWORD");?>:
			<input type="text" name="keyword" value="<?php echo (strlen(trim($this->keyword)) > 0 ?$this->keyword:"");?>" />
			<input type="button" name="go" value="<?php echo JText::_("DSSEARCH");?>" onClick="document.adminForm.task.value=''; document.adminForm.submit( );"/>
		</div>
		<div style="padding-bottom:0.5em">
			<?php echo JText::_("VIEWORDERSSTATUS");?>:
			<select name="status" onchange="document.adminForm.task.value=''; document.adminForm.submit();">
	...
	<?php
		for ($i = 0; $i < $n; $i++):
			...
			$link = JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&task=edit&cid[]=".$id.$on_link);
			$prodlink = JRoute::_("index.php?option=com_digistore&controller=digistoreProducts&task=edit&cid[]=".$license->productid);
			$customerlink = JRoute::_("index.php?option=com_digistore&controller=digistoreCustomers&task=edit&cid[]=".$license->userid);
	...
	?>
		<tr class="row<?php echo $k;?>">
				<td>
						<?php echo $checked;?>
					<input type="hidden" id="licenses<?php echo $license->id;?>" name="licenses<?php echo $license->id;?>" value="<?php echo digistoreAdminModeldigistoreLicense::getLicensesInOrder($license->orderid);?>" />
			</td>
			...
			<td nowrap="nowrap"><?php
					if ($license->duration_count != -1) {
						echo digistoreAdminHelper::getDurationType($license->duration_count, $license->duration_type);
					} else {
	...
	<?php
			...
			$regdomain_link = JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&task=register&cid[]=".$id."&Itemid=".$Itemid);
			$regdevdomain_link = JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&task=devregister&cid[]=".$id."&Itemid=".$Itemid);
				$download_link = JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&task=download&cid[]=".$id."&Itemid=".$Itemid);
			$devdownload_link = JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&task=devdownload&cid[]=".$id."&Itemid=".$Itemid);
	...
	?>
			...
			<td align="center">
				<?php
					if($configs->hour24format == "0"){
						...
						$date_format = date(($configs->time_format)." h:i:s A", $int_date);
						echo $date_format;
					}
					else{
						$int_date = strtotime($license->purchase_date);
						$date_format = date(($configs->time_format)." h:i:s", $int_date);
						echo $date_format;
					}
				?>
			</td>
			...
			<td align="center">
				...
				{
					// Chargeback
					echo '<span style="color:#ff0000;">' . JText::_("LICENSE_CHARGEBACK") . ' - ' . digistoreAdminHelper::format_price($license->cancelled_amount, $configs->currency, true, $configs) . '</span>';
				}
				elseif ($license->cancelled == 2)
				{
					// Refund
					echo '<span style="color:#ff0000;">' . JText::_("LICENSE_REFUND") . ' - ' . digistoreAdminHelper::format_price($license->cancelled_amount, $configs->currency, true, $configs) . '</span>';
				}
				else
				{
					echo '-';
