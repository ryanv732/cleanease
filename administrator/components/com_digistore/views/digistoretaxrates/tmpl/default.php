...
<td align="left">
	<form action="index.php" name="adminFormsearch" method="post">
		...
		<input type="hidden" name="controller" value="digistoreTaxRates" />
	</form>
</td>
...
<!--<form action="index.php" name="pluginFileForm" method="post"	enctype="multipart/form-data" >-->
	<form action="index.php" name="adminFormfile" method="post" enctype="multipart/form-data">
		...
		(<a href="index3.php?option=com_digistore&controller=digistoreTaxRates&task=viewsample&no_html=1" target="_blank" ><?php echo JText::_("DSVIEWSAMPLE");?></a>)
	  	<input type="hidden" name="option" value="com_digistore" />
...
//	for ($i = 1; $i <= $n; $i++):
	...
	foreach ($this->rates as $i => $v):
		...
		$link = JRoute::_("index.php?option=com_digistore&controller=digistoreTaxRates&task=edit&cid[]=".$id);
