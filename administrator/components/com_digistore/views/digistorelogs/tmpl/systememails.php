	...
	<legend><?php echo JText::_('VIEWPRODEMAILS'); ?></legend>
		<form id="adminForm" name="adminForm" action="index.php" method="post">
			...
			<table>
				...
				<tr>
					<td align="right">
						<a class="modal digi_video" rel="{handler: 'iframe', size: {x: 750, y: 435}}" href="index.php?option=com_digistore&controller=digistoreAbout&task=vimeo&id=38437540">
							<img src="<?php echo JURI::base(); ?>components/com_digistore/assets/images/icon_video.gif" class="video_img" />
							<?php echo JText::_("COM_DIGISTORE_VIDEO_SYSTEM_EMAILS"); ?>				  
						</a>
					</td>
				</tr>
			</table>
									...
									<?php
										if(intval($email["emailid"]) == 0){
											if($email["emailname"] == "New Customer Email" || $email["emailname"] == "New purchase emails"){
												echo '<a href="index.php?option=com_digistore&controller=digistoreConfigs&task2=email">'.$email["emailname"].'</a>';
											}
											elseif($email["emailname"] == "Product Email"){
												echo '<a href="index.php?option=com_digistore&controller=digistoreProducts&task=edit&cid[]='.$email["productid"].'&tab=10">'.$email["emailname"].'</a>';
											}
											...
											echo '<a href="index.php?option=com_digistore&controller=digistoreEmailreminders&task=edit&cid[]='.$email["emailid"].'&tab=8">'.$email_name.'</a>';
										}
									?>
								</td>
								<td>
									<?php
										...
										echo '<a href="index.php?option=com_digistore&controller=digistoreCustomers&task=edit&cid[]='.$email["userid"].'">'.$user_details["0"]["firstname"].' '.$user_details["0"]["lastname"].'</a>'.' ('.$user_details["0"]["username"].')';
									?>
								</td>
								<td>
									<?php
								...
								<td>
									<?php 
										...
										if(($today_day == $email_day) && ($today_month == $email_month) && ($today_year == $email_year)){
											echo JText::_("VIEWSTATTODAY")." (".date($configs->time_format, $email_date_int).") ".JText::_("DIGI_AT")." ".date("H:i:s A", $email_date_int)." (PST)";
										}
										elseif((($today_day-1) == $email_day) && ($today_month == $email_month) && ($today_year == $email_year)){
											echo JText::_("VIEWSTATYESTODAY")." (".date($configs->time_format, $email_date_int).") ".JText::_("DIGI_AT")." ".date("H:i:s A", $email_date_int)." (PST)";
										}
										else{
											echo strftime("%A", $email_date_int)." (".date($configs->time_format, $email_date_int).") ".JText::_("DIGI_AT")." ".date("H:i:s A", $email_date_int)." (PST)";
										}
									?>
								</td>
								<td>
									<?php
										echo "<a rel=\"{handler: 'iframe', size: {x: 700, y: 500}}\"  class=\"modal\"  href=\"index.php?option=com_digistore&controller=digistoreLogs&task=editEmail&id=".$email["id"]."&tmpl=component\">".JText::_('DIGI_VIEW_EMAIL')."</a>";
									?>
								</td>
							</tr>
					<?php
			...
			<input type="hidden" name="controller" value="digistoreLogs" />
		</form>
</fieldset>
