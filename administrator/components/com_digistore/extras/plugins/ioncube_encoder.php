...
class ioncube {
	...
	function getEncPath () {
/*		global $database;
		$query = "select value from #__digistore_plugins where name='".get_class($this)."'";
...
//			.$mainframe->getCfg( 'absolute_path' )."/components/com_digistore/tmp/".$now."/origin/$file "
//			." -o ".$mainframe->getCfg( 'absolute_path' )."/components/com_digistore/tmp/".$now."/encoded/$method/$file";
			.$srcPath.$file
			." -o ".$dstPath.$file;
		$result = exec( $command );
//echo ($command."<br/>");
