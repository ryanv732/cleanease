...
$x = ( unserialize(base64_decode($tax_rate)));
		...
		if (isset($tax['type']) && strlen ($tax['type']) > 0){
			...
			$scountry = $configs->country; //shop's location
			$sperson = $configs->person; //shop's owner a person?
			$staxnum = $configs->taxnum; //shop's owner tax number

			$sql = "select eumember from #__digistore_states where country='".$scountry."' limit 1";
			$db->setQuery($sql);
			$seumember = $db->loadResult();//is shop in eu?
			$sql = "select eumember from #__digistore_states where country='".$ccountry."' limit 1";
			$db->setQuery($sql);
...
//echo "D";
			...
			if ($cargevat == 0) {//charge nothing
				$tax['type'] =  '';
					...
					$tax['value'] = digistoreHelper::format_price($vat_tax, $tax['currency'], false, $configs);//sprintf($price_format,$vat_tax);// . " " . $tax['currency'] . '<br>';
			}
