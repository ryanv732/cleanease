...
//print_r($pconf);
		...
		if ( $configs->tax_option == 'tax_option_1' ) {
			  if ( ($configs->country === $cust_info->_customer->country) && ($configs->state === $cust_info->_customer->state )) {
					  $pay_flag = true;
			  } 
		}
		...
		if ( $configs->tax_option == 'tax_option_2' ) {
				if ( $configs->country === $cust_info->_customer->country ) {
						$pay_flag = true;
				} 
		}
		...
		//tax option 3 - for any customer
			...
			if ( $pay_flag && ($cust_info->_user->id > 0) || $configs->tax_option == 'tax_option_3' ) {
				   		if ($tax_rate != 0 ) {
								...
								$tax['value'] = digistoreHelper::format_price($state_tax, $tax['currency'], false, $configs);////sprintf($price_format,$state_tax) ;//. " " . $tax['currency'] . '<br>';
						}
			  /*
			//vat tax - calculates in a different way
			}*/
