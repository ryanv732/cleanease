 ...
 */
//HandleDigistorePlugins::registerPlugin ($plugin);
...
class offline {
	...
	function deleteCurrency() {
	   ...
	   $database = JFactory::getDBO();
		...
		$sql = "DELETE FROM #__digistore_currencies WHERE plugname='" . get_class($this) . "'";
	...
	function getFEData($items, $tax, $redir, $profile, $plugin, $configs) {
		...
		$content =	'<br />
			<form action="'.$mosConfig_live_site.'/index.php?option=com_digistore&controller=digistoreCart&task=returnPayment&plugin=offline&sid='.$sid.'&Itemid='.$_Itemid.'" method="post" name="paymentForm1"></form>
			<script>
