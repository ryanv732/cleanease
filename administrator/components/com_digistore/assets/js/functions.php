...
function addNote(){
	...
	$sql = "insert into #__digistore_licenses_notes (`lic_id`, `notes`, `expires`) values (".intval($licid).", '".trim(addslashes($text))."', '".trim($expire)."')";
	$database->setQuery($sql);
	$database->query();

	$sql = "select max(id) from #__digistore_licenses_notes";
	$database->setQuery($sql);
	$database->query();
