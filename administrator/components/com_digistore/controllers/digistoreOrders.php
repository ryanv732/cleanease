...
class digistoreAdminControllerdigistoreOrders extends digistoreAdminController
{
...
//		$this->registerTask ("add", "edit");
	...
	function saveorder() {
		...
		$this->setRedirect('index.php?option=com_digistore&controller=digistoreOrders');
	}
	...
	function saveCustomer() {
		...
		if ($result = $customer_model->store($error) ) {
			...
			if ($id > 0) {
				$this->setRedirect('index.php?option=com_digistore&controller=digistoreOrders&task=prepereNewOrder&userid='.$id);
			}
		} else {
			...
			$model = $this->getModel( "digistoreConfig" );
			$view->setModel( $model );
			$model = $this->getModel('digistoreCustomer');
			$view->setModel( $model );
	...
	function newCreateCustomer() {
		...
		if(trim($username) == ""){
			$this->setRedirect('index.php?option=com_digistore&controller=digistoreOrders&task=checkcreateuser&usertype=3', JText::_("DIGI_ENTER_VALID_USERNAME"), "notice");
		}
		...
		if (!empty($user->id)) {
			...
			if (!empty($cust->firstname)) {
				$this->setRedirect('index.php?option=com_digistore&controller=digistoreOrders&task=prepereNewOrder&userid='.$user->id);
			} else {
				$view->newCustomer();
			}
		} else {
	...
	function save()
	{
		...
		$link = "index.php?option=com_digistore&controller=digistoreOrders";
	...
	function cycleStatus(){
		...
		$link_orders = "index.php?option=com_digistore&controller=digistoreOrders";
