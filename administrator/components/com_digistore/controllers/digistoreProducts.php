...
class digistoreAdminControllerdigistoreProducts extends digistoreAdminController {
	var $_model = null;
...
// 		JRequest::setVar ("view", "digistoreProducts");
	...
	function uploadimages() {
		...
		$path_image = JPATH_ROOT . DS . "images" . DS . "stories" . DS .  "digistore" . DS . "products" . DS;
	...
	function edit () {
		...
		$model = $this->getModel("digistorePlain");
		$view->setModel($model);

		$model = $this->getModel("digistoreEmailreminder");
		$view->setModel($model);
	...
	public function save(){
		...
		switch ($task){
			case 'save':
			case 'save2copy':
				$save_url =  "index.php?option=com_digistore&controller=digistoreProducts".$this->csel;
			...
			case 'apply':
				...
				$apply_url = "index.php?option=com_digistore&controller=digistoreProducts&task=edit&cid[]=".$product_id."&tab=".$tab;
				$this->setRedirect($apply_url, $msg);
				break;
			case 'save2new':
				$producttype = JRequest::getVar('domainrequired',0);
				$redirect_url = 'index.php?option=com_digistore&controller=digistoreProducts&task=add&producttype='.$producttype;
...
// 		http://localhost/obexts/j30/administrator/index.php?option=com_digistore&controller=digistoreProducts&task=add&producttype=0
	}
	...
	function remove () {
		...
		$link = "index.php?option=com_digistore&controller=digistoreProducts".$this->csel;
	...
	function moveImageDown(){
		...
		$this->setRedirect("index.php?option=com_digistore&controller=digistoreProducts&task=edit&cid[]=".$cid);
	}
	...
	function deleteImages(){
		...
		$product_id = JRequest::getVar('id','');
		$apply_url = "index.php?option=com_digistore&controller=digistoreProducts&task=edit&cid[]=".$product_id."&tab=6";
		$this->setRedirect($apply_url, $msg);
	}
	...
	function copyProduct()
	{
		...
		if($response === TRUE)
		{
			...
			$this->setRedirect("index.php?option=com_digistore&controller=digistoreProducts&catid=$catid", $msg);
		}
		else
		{
			$msg = JText::_("DIGI_COPY_ERROR");
			$this->setRedirect("index.php?option=com_digistore&controller=digistoreProductss&catid=$catid", $msg, "notice");
		}*/
		$this->listProducts();
	}
