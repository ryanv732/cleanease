...
class digistoreAdminControllerdigistoreLicenses extends digistoreAdminController {
	public $_model = null;
	...
	function __construct () {
		...
		$this->_model = $this->getModel("digistoreLicense"); 
		$my = JFactory::getUser();
	...
	function deleteNote(){
		...
		$cid = intval($cid["0"]);
		$link = "index.php?option=com_digistore&controller=digistoreLicenses&task=edit&cid[]=".$cid;
		$this->setRedirect($link);
	}

	function selectnote() {

		$view = $this->getView("digistoreLicenses", "html");
		$view->setModel($this->_model, true);

		$view->setLayout("selectnote");

		$model = $this->getModel('digistoreCategory');
	...
	function edit () {
		...
		$model = $this->getModel("digistoreConfig");
		$view->setModel($model);
		$model = $this->getModel("digistoreProduct");
		$view->setModel($model);
		$model = $this->getModel('digistoreCategory');
		$view->setModel($model, false);		 
	...
	function add () {
		...
		$this->setRedirect( "index.php?option=com_digistore&controller=digistoreOrders&task=add" );
	}
	...
	function save () {
		...
		$link = "index.php?option=com_digistore&controller=digistoreLicenses";
		...
		if(JRequest::getVar('task','') == 'save'){
			$link = "index.php?option=com_digistore&controller=digistoreLicenses".$on_link;
		}
		else{
			$lic_id = JRequest::getVar('id','');
			$link = "index.php?option=com_digistore&controller=digistoreLicenses&task=edit&cid[]=".$lic_id.$on_link;
		}
	...
	function remove () {
		if (!$this->_model->delete()) {
			...
			digistoreAdminHelper::expireUserProduct($this->_customer->_user->id);
		}
	...
	function register () {
		...
		$license =  $this->getModel("digistoreLicense");
...
//		}
		...
		if (!$res ) $add = "i";
		$this->setRedirect(JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&add=".$add."&Itemid=".$Itemid, false ));
	}
...
//		$uid = $this->_customer->_user->id ;
		...
		$product = $this->getModel("digistoreProduct");
		...
		if ($product->main_zip_file){//product is encoded
			...
			if ( (JRequest::getVar('submitted','','post') == '') || (JRequest::getVar('phpVersions','','post') == '') || (JRequest::getVar('platforms','','post') == '') || (JRequest::getVar('licenseonly','0','post') == "1") ) {			
				...
				$plugin_handler = $this->getModel("digistorePlugin");
				$encoders = $plugin_handler->getEncoders();
				if (empty($encoders)) {
					echo JText::_('DIGISTORE_ENCODER_PLUGINS_NOT_INSTALLED');
					return;
				}			
				...
				$this->setRedirect(JRoute::_("index.php?option=com_digistore&controller=digistoreLicenses&tmpl=component&no_html=1&task=getProduct&id=".$license->id ));
			...
			$this->setRedirect("index.php?option=com_digistore&controller=digistoreLicenses&tmpl=component&no_html=1&task=getProduct&id=".$license->id );
...
//		$uid = $this->_customer->_user->id ;
		...
		if ($configs->directfilelink != 0) {
			$site=DigistoreHelper::getLiveSite();
			...
			digistoreAdminHelper::CreateIndexFile($usermainDir);	   
			
			$preDir = JPATH_ROOT.DS."components/com_digistore/download/".$uid."/";			
			digistoreAdminHelper::CreateIndexFile($preDir);			

			$this->setRedirect($site."/components/com_digistore/download/".$uid."/".$product->file );
			...
			$prepDir = JPATH_ROOT.DS."components/com_digistore/download/".$this->_customer->_user->id."/";
	...
	function decode_license() {
		...
		$dir = $mosConfig_absolute_path . "/administrator/components/digistore_product_uploads/tmp/" ;
		$dec_file = $dir . "license.txt";

		$decode_method = JRequest::getVar('decode_method', '', 'request');
		$sql = "SELECT * FROM #__digistore_plugins p, #__digistore_plugin_settings s WHERE name='{$decode_method}' and p.id=s.pluginid";
...
*/
			...
			$sql = "SELECT * FROM #__digistore_plugins WHERE published='1' AND type='encoding'";
			$database->setQuery($sql);
			$encoders = $database->loadObjectList();
	...
	function changeCategory(){
		...
		$sql = "select p.`id`, p.`name` from #__digistore_products p, #__digistore_product_categories c where p.id=c.productid and c.catid=".intval($categ_id)." order by p.`name` asc";
		$db->setQuery($sql);
		$db->query();
