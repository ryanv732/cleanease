...
class TabledigistoreLicense extends JTable
{
	...
	function TabledigistoreLicense( &$db )
	{
		parent::__construct( '#__digistore_licenses', 'id', $db );
	}
	...
	function load( $id = 0, $reset = true )
	{
		parent::load( $id );
		$db = JFactory::getDBO();
		$sql = "select firstname, lastname from #__digistore_customers where id='" . $this->userid . "'";
		$db->setQuery( $sql );
		...
		$sql = "select name from #__digistore_products where id='" . $this->productid . "'";
		$db->setQuery( $sql );
		$res = $db->loadObjectList();
		if ( count( $res ) > 0 ) {
